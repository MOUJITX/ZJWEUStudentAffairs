<template>
  <div>
    <div style="margin: 5px 0">
      <div>
        当前学年批次：<el-tag>{{ dataForm.pc }}</el-tag>
      </div>
      <div>
        用工单位：<el-tag>{{ dataForm.bm }}</el-tag>
      </div>
      <div>
        当前可录用人数：
      </div>
      <div>【钱塘校区】<el-tag>已录{{ departNum.qtly }} / 余{{ departNum.qtsy }} / 总{{ departNum.qtrs }}</el-tag>
      </div>
      <div>【南浔校区】<el-tag>已录{{ departNum.nxly }} / 余{{ departNum.nxsy }} / 总{{ departNum.nxrs }}</el-tag>
      </div>
      <div>（已录用人数不含“未通过审核”和“被其他部门录用”人数）
      </div>
      <span>录用名单录入：</span>
      <el-button style="margin-right: 10px" type="primary" @click="add()">手动添加</el-button>
      <el-button type="primary" @click="scanCodeTip">扫码录入</el-button>
    </div>
    ...
    <el-drawer
      ...
      <el-form label-width="100px">
        ...
        <el-form-item label="学院">
          <el-input v-model="searchForm.xy" style="width: 90%;" clearable placeholder="请输入学院" @input="load()"></el-input>
        </el-form-item>
        ...
        <el-form-item label="录用状态">
          <el-select v-model="searchForm.zt" style="width: 90%;" clearable placeholder="请选择录用状态" @input="load()">
    ...
    <div>
      ...
      <el-popconfirm  title="确认删除?" @confirm="delBatch()">
        <template #reference>
          <el-button :disabled="delButtonDisable" style="margin-right: 10px" type="danger">删除选中</el-button>
        </template>
      </el-popconfirm>
    </div>
    <el-table :data="tableData" style="z-index: 0" @selection-change="tableSelected">
      ...
      <el-table-column label="录用结果" prop="zt" width="200"></el-table-column>
...
export default {
  data() {
    return {
      ...
      searchForm: {
        ...
        "xh": "",
        "xm": "",
        "bm": JSON.parse(localStorage.getItem("UserInfo")).note,
        "pc": "",
        "zt": "",
      },
  ...
  methods: {
    ...
    confirmMenu() {
      this.$request.post('/menu/ConfirmAccess', this.$route.path).then(res => {
        if (res.code === '403') {
          ...
          this.getTerminfo()
          if (this.$route.fullPath.includes('qrcode')) {
            this.scanCode()
          }
        }
      })
    ...
    scanCodeTip(){
      this.$alert("请直接使用微信【扫一扫】扫描申请表上的二维码，每次扫描录入后，需要关闭页面继续使用【扫一扫】。录入过程中请保持账号为登录状态。","使用指南", {
        confirmButtonText: '确定'
      });
    },
    scanCode() {
      this.$request.post("/qgzxOffer/qrcode?bm=" + this.searchForm.bm, this.qrcode).then(res =>{
        this.$alert(res.data,res.msg, {
          confirmButtonText: '确定',
          callback: action => {
            this.load()
