<template>
  <div>
    <div style="margin-bottom: 10px">
      <el-button style="margin-right: 10px" type="primary" @click="$router.push('/Phone/QgzxDepart')">查看岗位
      </el-button>
      <el-button v-if="terminfo.dqzt === '0'" style="margin-right: 10px" type="primary"
                 @click="showWarning('未开放申请')">暂无申请权限：未开放申请
      </el-button>
      <el-button v-if="terminfo.dqzt === '1' && stuType === '0' " style="margin-right: 10px" type="primary"
                 @click="showWarning('非资助认定对象')">暂无申请权限：非资助认定对象
      </el-button>
      <el-button v-if="terminfo.dqzt === '1' && stuType !== '0'" style="margin-right: 10px" type="primary"
                 @click="add()">新增申请
      </el-button>
      <el-button v-if="terminfo.dqzt === '2'" style="margin-right: 10px" type="primary" @click="add()">新增申请
      </el-button>
      ...
      <div>注意事项：<span v-html="terminfo.zssm"/></div>
    </div>
    <el-table :data="tableData" style="z-index: 0" @selection-change="tableSelected">
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column align="center" label="学年批次" prop="sqxn" width="auto"/>
      <el-table-column align="center" label="审批进度" width="100">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <div><el-button size="mini" type="text" @click="handleEdit(scope.row.id)">申请查看</el-button></div>
          <div><el-button size="mini" type="text" @click="getPDF(scope.row.id)">导出申请表</el-button></div>
        </template>
      </el-table-column>
    </el-table>
    <div>
      ...
      <el-dialog :title="dialogSetting.title" :visible.sync="dialogSetting.show" width="100%">
        <el-form ref="dataForm" :disabled="dialogSetting.dis || dataForm.speed !== 'save'" :model="dataForm" label-width="100px" :rules="formRules">
          <el-form-item label="审批进度" prop="speed">
            <el-tag v-if="dataForm.speed === 'save'" type="info">暂未提交/学生暂存</el-tag>
          ...
          <el-form-item label="申请人">{{ dataForm.name }}（{{ dataForm.username }}）
          </el-form-item>
          <el-form-item label="学院班级">{{ dataForm.college }} {{ dataForm.classname }} {{ dataForm.campus }}
          </el-form-item>
          <el-form-item label="申请学年" prop="sqxn">
            {{ dataForm.sqxn }}
          </el-form-item>
          <el-form-item label="资质认定" prop="type">
            <el-tag v-if="dataForm.type === '3'">特别困难</el-tag>
            <el-tag v-if="dataForm.type === '2'">困难</el-tag>
            <el-tag v-if="dataForm.type === '1'">一般困难</el-tag>
            <el-tag v-if="dataForm.type === '0'">未认定</el-tag>
          </el-form-item>
          ...
          <el-form-item label="勤工经历" prop="lastdepart">
            <el-select v-model="dataForm.lastdepart" clearable filterable multiple placeholder="">
              <el-option
            ...
            <el-form-item label=" " style="font-weight: bold">本学年勤工助学申请信息</el-form-item>
          <el-form-item label="申请岗位1" prop="depart">
            <el-select v-model="dataForm.depart" clearable filterable placeholder="">
          ...
          <el-form-item label="申请岗位2" prop="departb">
            <el-select v-model="dataForm.departb" clearable filterable placeholder="">
              <el-option
          ...
          <el-form-item label="可用工作时长">单位：小时，填写整数，单日最长为8小时
            <p>
              周一　
          ...
          <el-form-item v-if="terminfo.pcdm === dataForm.sqxn" label="注意事项" style="font-weight: bold">
  ...
  methods: {
    ...
    add() {
      if (this.tableData.length !== 0 && this.tableData[0].sqxn === this.terminfo.pcdm) {
        this.$notify.error({
          title: "新增申请失败",
          message: "当前批次已存在一条申请记录，每轮申请批次最多仅可存在一条记录。"
        })
      } else {
        this.dataForm = {
          "id": "",
