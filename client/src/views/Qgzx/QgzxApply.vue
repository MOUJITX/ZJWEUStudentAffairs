<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-input v-model="searchForm.college" placeholder="请输入学院" style="width: 10%;margin-left: 20px" clearable @input="load" ></el-input>
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.$request.delete("/qgzxApply/del/" + id).then(res => {
        if (res.code === '200') {
          this.$notify.success({title: res.data})
          this.load()
        } else {
    ...
    delBatch() {
      ...
      this.$request.delete("/qgzxApply/del/batch", {data: this.selectedIds}).then(res => {
        if (res.code === '200') {
          this.$notify.success({title: res.data})
          this.load()
          this.delButtonDisable = true
